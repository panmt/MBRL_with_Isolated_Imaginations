...
class Dreamer(nn.Module):
  ...
  def _policy(self, obs, state, training):
    if state is None:
      ...
      latent, action = state
    embed = self._wm.encoder(self._wm.preprocess(obs), action)
    ...
    if self._config.rollout_policy:
      ...
      if self._config.sz_sparse:
        gate_feat = torch.cat([latent_free['deter_free'], latent_free['deter']], -1)
        gate_feat = self._wm.dynamics.prior_gate(gate_feat)
        prior_gate_data = networks.boundary_sampler(gate_feat)
        # prior_gate_data = prior_gate_data[:, 0].unsqueeze(-1)
        x = torch.cat([latent_free['stoch_free'], latent_free['stoch'] * prior_gate_data], -1)
        stoch_free = self._wm.dynamics.sz_concat(x)
        latent_free['stoch_free'] = stoch_free
