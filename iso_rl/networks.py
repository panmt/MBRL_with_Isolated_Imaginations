...
class RSSM(nn.Module):
  ...
  def __init__(
    ...
    for i in range(self._layers_input):
      ...
      if i == 0:
        ...
        elif config.min_free:
          inp_layers_free.append(nn.Linear(inp_dim, self._hidden))
        else:
    ...
    if self._config.sz_sparse:
      self.prior_gate = PriorBoundaryDetector(input_size=2*self._deter)
      self.sz_concat = nn.Linear(2*self._stoch, self._stoch)
  ...
  def obs_step(self, prev_state, prev_action, embed, sample=True):
    ...
    if self._config.min_free:
      post['deter_free_pos'] = prior['deter_free_pos']
      post['deter_free_neg'] = prior['deter_free_neg']
    if self._config.max_action:
      post['deter_action_neg'] = prior['deter_action_neg']
    if self._config.sz_sparse and ('gate' in prior.keys()):
      post['gate'] = prior['gate']
  ...
  def img_step(self, prev_state, prev_action, embed=None, sample=True, only_free=False):
    ...
    if not only_free:
      ...
      if self._shared:
        ...
        if self._config.max_action:
          x_action_neg = torch.cat([prev_stoch, -prev_action], -1)

      x = self._inp_layers(x)
      if self._config.max_action:
         x_action_neg = self._inp_layers(x_action_neg)
      for _ in range(self._rec_depth): # rec depth is not correctly implemented
        deter = prev_state['deter']
        if self._config.max_action:
          x_action_neg, deter_action_neg = self._cell(x_action_neg, [deter])
          deter_action_neg = deter_action_neg[0]
      ...
      if sample:
        ...
        stoch = self.get_dist(stats, free=False).mode()
      if self._config.max_action:
        prior = {'stoch': stoch, 'deter': deter, 'deter_action_neg': deter_action_neg, **stats}
      else:
        prior = {'stoch': stoch, 'deter': deter, **stats}
      
    prev_stoch_free = prev_state['stoch_free']
    if self._config.sz_sparse and not only_free:
        feat_gate = torch.cat([prev_state['deter_free'], prev_state['deter']], -1)
        prior_gate = self.prior_gate(feat_gate)
        prior_gate_data = boundary_sampler(prior_gate)
        x_concat = torch.cat([prev_state['stoch_free'], prev_state['stoch'] * prior_gate_data], -1)
        prev_stoch_free = self.sz_concat(x_concat)
    if self._shared:
      if embed is None:
    ...
    if self._config.min_free and not only_free:
      x_pos_free = torch.cat([x, prev_action], -1)
      x_neg_free = torch.cat([x, -prev_action], -1)
      zero_action = torch.zeros(prev_action.shape).to(self._device)
      x_free = torch.cat([x, zero_action], -1)
      x_pos = self._inp_layers_free(x_pos_free)
      x_neg = self._inp_layers_free(x_neg_free)
      x = self._inp_layers_free(x_free)

      for _ in range(self._rec_depth): # rec depth is not correctly implemented
        assert self._rec_depth == 1
        deter_free = prev_state['deter_free']
        x_pos, deter_free_pos = self._cell_free(x_pos, [deter_free])
        x_neg, deter_free_neg = self._cell_free(x_neg, [deter_free])
        x, deter_free = self._cell_free(x, [deter_free])
        deter_free_pos = deter_free_pos[0]  # Keras wraps the state in a list.
        deter_free_neg = deter_free_neg[0]
        deter_free = deter_free[0]
      ...
      prior.update({'stoch_free': stoch_free, 'deter_free': deter_free, 'deter_free_pos': deter_free_pos, 'deter_free_neg': deter_free_neg, **stats_free})
      
      if self._config.sz_sparse:
        prior.update({'gate': prior_gate_data})
      
      return prior

    assert not self._discrete and not self._shared, 'not implemented!'
    if self._config.min_free:
      zero_action = torch.zeros(tuple(x.shape[:-1])+(2,)).to(self._device)
      x = torch.cat([x, zero_action], -1)
...
class ConvEncoder(nn.Module): 
  ...
  def __init__(self, grayscale=False,
    ...
    for i, kernel in enumerate(self._kernels):
      ...
      if i > 2:
        ...
        if config.encoder_add_action:
          layers_action.append(nn.Conv2d(inp_dim+self._config.num_actions, depth, kernel, 2))
          layers_free.append(nn.Conv2d(inp_dim+self._config.num_actions, depth, kernel, 2))
        else:
          layers_action.append(nn.Conv2d(inp_dim, depth, kernel, 2))
          layers_free.append(nn.Conv2d(inp_dim, depth, kernel, 2))
        layers_action.append(act())
  ...
  def __call__(self, obs, action, bg=False):
    ...
    if bg:
      ...
      return x_back
    if self._config.encoder_add_action:
      width = x.shape[-1]
      action = action.reshape([-1, action.shape[-1]])
      action = action.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, width, width)
...
class LinearLayer(nn.Module):
    ...
    def forward(self, input_data):
        return nn.Sigmoid()((self.linear(input_data)))


def boundary_sampler(log_alpha):
    sample_data = (log_alpha >= 0.5)
    sample_data = sample_data.detach() + (log_alpha - log_alpha.detach())
